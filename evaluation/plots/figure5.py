...
#npzfile_min_ptm = np.load("../output/evaluated_complex_data_50_simulations_min_ptm.npz")
npzfile_min_ptm = np.load("../output/evaluated_complex_data_100_simulations_min_ptm_36ppm.npz")
npzfile_min_both = np.load("../output/evaluated_complex_data_100_simulations_min_both_36ppm.npz")
...
metric_min_ptm3 = pd.DataFrame({"PTM pattern": npzfile_min_ptm["arr_4"].flatten(), "mass_shift": modform_distribution.mass.tolist()*repeats_min_ptm})
metric_min_ptm3["PTM pattern"].mask(metric_min_ptm3["PTM pattern"] == 1.0, "true positive", inplace=True)
metric_min_ptm3["PTM pattern"].mask(metric_min_ptm3["PTM pattern"] == 0.0, "false positive", inplace=True)
metric_min_ptm3["PTM pattern"].mask(metric_min_ptm3["PTM pattern"].isna(), "not detected", inplace=True)
ptm_pattern_grouped_min_ptm3 = metric_min_ptm3.groupby(["mass_shift", "PTM pattern"]).size().unstack(fill_value=0)
...
metric_min_both3 = pd.DataFrame({"PTM pattern": npzfile_min_both["arr_4"].flatten(), "mass_shift": modform_distribution.mass.tolist()*repeats_min_both})
metric_min_both3["PTM pattern"].mask(metric_min_both3["PTM pattern"] == 1.0, "true positive", inplace=True)
metric_min_both3["PTM pattern"].mask(metric_min_both3["PTM pattern"] == 0.0, "false positive", inplace=True)
metric_min_both3["PTM pattern"].mask(metric_min_both3["PTM pattern"].isna(), "not detected", inplace=True)
ptm_pattern_grouped_min_both3 = metric_min_both3.groupby(["mass_shift", "PTM pattern"]).size().unstack(fill_value=0)
...
print(ptm_pattern_grouped_min_ptm[0.75<1-(ptm_pattern_grouped_min_ptm["not detected"]/amount_simulations)].shape[0], 
      "mass shifts detected out of", amount_mass_shifts_min_ptm)
print(ptm_pattern_grouped_min_ptm[0.75<(ptm_pattern_grouped_min_ptm["true positive"]/amount_simulations)].shape[0], 
      "true positive PTM pattern predictions", amount_mass_shifts_min_ptm, "mass shifts")


print("Results for objective with min_both:")
print(ptm_pattern_grouped_min_both[0.75<1-(ptm_pattern_grouped_min_both["not detected"]/amount_simulations)].shape[0], 
      "mass shifts detected out of", amount_mass_shifts_min_both)
print(ptm_pattern_grouped_min_both[0.75<(ptm_pattern_grouped_min_both["true positive"]/amount_simulations)].shape[0], 
